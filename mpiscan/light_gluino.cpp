...
class light_gluino_builder : public point_builder {
public:

    light_gluino_builder()
            : point_builder(49),
              m0_dist(100, 10000),
              mhf_dist(100, 2000),
              m3alpha_dist(1.0 / 6.0, 1.0),
              a0_by_m0_dist(-8, 8),
              tanb_dist(2, 62) {
    ...
    softsusy_opts operator()() {
        ...
        double m3alpha = m3alpha_dist(gen());
...
private:
    ...
    uniform_real_distribution<> m3alpha_dist;
...
int main(int argc, char **argv) {
    ...
    fstream writer;
    string myid;
    light_gluino_builder light_gluino;

    myid = light_gluino.mpi_setup(&writer);
...
//	superiso_driver superiso;
    ...
    for (ulonglong npoints = 1ULL; npoints <= 1000000000ULL; npoints++) {
        ...
        if (0 == npoints % status_interval) {
            ...
            << light_gluino.get_time() << " seconds." << endl;

            if (0 == npoints % (10ULL * status_interval))
                status_interval *= 10ULL;
        }

        auto sugra = light_gluino();
        ...
        try {
            ...
            if (light_gluino.filter_point(m))
                continue;
