...
public:
	...
	model(std::string _model_name, std::map<std::string,double> _slha_data, susy_model _model_type, const std::string &obs_dat_line);
	...
	void set_datum(const std::string &key, double value) { slha_data[key] = value; build_spectrum(); } // happy to overwrite 
