...
const double default_alpha_s = 0.1184;
const double default_alpha_em_inv = 127.933;
const double default_mtop = 173.5;
const double default_mbmb = 4.18;

point_opts::point_opts(int argc, char**argv)
:pars(0),mgut_guess(2e16),alpha_s(default_alpha_s),alpha_em_inv(default_alpha_em_inv),mtop_pole(default_mtop),mbmb(default_mbmb)
{
	try {
		opthandle(argc,argv);
...
int point_opts::opthandle(int argc, char** argv)
{
...
void point_opts::make_pars(int argi, char** argv)
{
...
SoftSusyBCFnPtr point_opts::get_bcs() const
{
	if (nusugra)
