...
#ifdef NUSUGRA
// non-universal
const int extra_params = 2;
const double delta_h1_min = -(1.0-1e-6);
const double delta_h1_max = 5.0;
const double delta_h2_min = -(1.0-1e-6);
const double delta_h2_max = 5.0;
#else
...
#ifdef NUSUGRA
	// non-universal
	double delta_h1 = get_mapped(Cube[8], delta_h1_min, delta_h1_max, false);
	double delta_h2 = get_mapped(Cube[9], delta_h2_min, delta_h2_max, false);
...
void log_like_request(double *Cube, int &ndim, int &npars, double &lnew, void *context)
{
...
#else
	std::cerr << "Now trying " << m0 << " " << mhf << " " << a0 << " " << tb << " 2e16 1 " << mb << " " << m_top << " " << alpha_s << " " << alpha_em_inv << " ; dh1=" << delta_h1 << " ; dh2=" << delta_h2 << " ..."; // don't flush!
#endif
	...
	pars(21) = boost::math::pow<2>(m0*(1.0+delta_h1)); //mh1^2
	pars(22) = boost::math::pow<2>(m0*(1.0+delta_h2)); //mh2^2
