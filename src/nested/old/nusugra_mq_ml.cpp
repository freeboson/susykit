...
#ifdef NUSUGRA
// non-universal
const int extra_params = 1;
const double delta_ml_min = -1.0;
const double delta_ml_max = 0;
#else
...
#ifdef NUSUGRA
	// non-universal
	double delta_ml = get_mapped(Cube[8], delta_ml_min, delta_ml_max, false);
...
void log_like_request(double *Cube, int &ndim, int &npars, double &lnew, void *context)
{
...
#ifdef NUSUGRA
	double delta_ml = Cube[8];
#endif

#ifndef NUSUGRA
	std::cerr << "Now trying " << m0 << " " << mhf << " " << a0 << " " << tb << " 2e16 1 " << mb << " " << m_top << " " << alpha_s << " " << alpha_em_inv << " ..."; // don't flush!
#else
	std::cerr << "Now trying " << m0 << " " << mhf << " " << a0 << " " << tb << " 2e16 1 " << mb << " " << m_top << " " << alpha_s << " " << alpha_em_inv << " ; dml=" << delta_ml << " ..."; // don't flush!
#endif
	...
	pars(31) = m0*(1+delta_ml); //meL
	pars(32) = m0*(1+delta_ml); //mmuL
	pars(33) = m0*(1+delta_ml); //mtauL
	pars(34) = m0*(1+delta_ml); //meR
	pars(35) = m0*(1+delta_ml); //mmuR
	pars(36) = m0*(1+delta_ml); //mtauR
