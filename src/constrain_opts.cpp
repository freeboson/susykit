...
void print_usage(const string &s)
{
	cerr 	<< "Constrain-1.1 by Sujeet Akula" << endl
		...
		<< endl
		<< "Usage: " << s << " [-alpf] [ -L <loglike.conf> ] [ -M | [ -m | --obs <observables.cdb> ] ] [EXPR1 [EXPR2 [...]]] [inputfile.sdb [outputfile.mdb]]" << endl
		...
		<< "\t" << "CLINE is \"<param> LIMIT\" and LIMIT is [x] <lower | #> <upper | #> (you will want to pass CLINE as a single string)" << endl
		<< "\t" << "where [lower,upper] is the relevant range imposed on param, and x switches from inclusive (default) to exclusive." << endl
		<< "\t" << "The codes for the CLINES are in dict.h; feel free to adjust them to your comfort." << endl
		<< "\t" << "Special CLINE's are hacks, but the codes include: NLSP, LSQ, and LSL. (Look in perform_lookup.cpp.)" << endl

		<< endl
		<< "Example: " << s << " --obs observables.cdb --cut \"m_g 100 800\" --output \"Omega # 0.14\" in.sdb" << endl
		<< "Here, models will be read from in.sdb, and the observables will be read in from observables.cdb. Since only " << endl
		<< "one filename is given at the end, models that have a gluino mass betwen 100 and 800 GeV and relic density less " << endl
		<< "than or equal to 0.14 will be printed to stdout (feel free to redirect to a file)." << endl
		<< endl
		<< "Example: " << "bzcat in.mdb.bz2 | " << s << " -m --cut \"m_g 100 800\" --output \"Omega # 0.14\" | bzip2 --best > out.mdb.bz2" << endl
		...
		<< endl
		<< "\t\t" << "-L, --likeconfig=likelihood.conf, this specifies the configuration file for the likelihood function (some lookups may need --obs or -m)" << endl
		<< endl
		<< "\t" << "Constraints:" << endl
		<< "\t\t" << "-x, --cut=CLINE, apply limits defined in CLINE to an SLHA datum" << endl
		<< "\t\t" << "-X, --chi2=LIMIT, apply limits defined in LIMIT (as in CLINE, without <param>) to chi^2 = -2ln(like) (needs -L)" << endl
	...
	while (-1 != (opt=getopt_long(argc, argv, optcstr, longopts, &longindex)))
	{
		switch(opt)
		{
			...
			case 'B':
				...
				ss.str("");
				ss.clear();
				ss << susy_dict::observable::bsgnlo << (bsgnlo_accepted + bsgnlo_delta - bsgnlo_lim) << " " << (bsgnlo_accepted + bsgnlo_delta + bsgnlo_lim);
				gopts->constraints.push_back(constraint( model_lookup(model_lookup::output,get_cline_code(ss.str())), ss.str()));

				gopts->constraints.push_back(constraint( model_lookup(model_lookup::output, susy_dict::observable::bsmumu), susy_dict::observable::bsmumu + string(" # 4.5e-9"))); // combined LHC limit
				gopts->constraints.push_back(constraint( model_lookup(model_lookup::output, susy_dict::observable::gmuon), susy_dict::observable::gmuon + string(" -11.4e-10 9.4e-9"))); // Brookhaven
				break;
