...
double hepstats::likedatum::operator()(const model &m, bool *unlikely) const
{
	...
	double theoretical_uncertainty = (theory_percent_error)
	...
	*unlikely = false;
	...
	switch (dist)
	{
		case likedist::Gaussian:
			loglikeval = -std::pow(theoretical_value - exp_value,2.0)/(std::pow(theoretical_uncertainty,2.0) + std::pow(exp_uncertainty,2.0))/2.0;
			break;
		...
		case likedist::Lower:
			...
			if (theory_uncertainty <= 0.0) // this is borked... do a step-fn
			{
...
//					std::cerr << "Unlikely! " << lookup.get_code() << "=" << theoretical_value << std::endl;
			...
			{
				loglikeval = get_95cl_loglike(delta, unlikely);
			}
			break;
...
double hepstats::likedatum::get_95cl_loglike(double delta, bool *unlikely) const
{
	// this is based on SmearedBound() in SuperBayes, source/calclike.f90
	// which in turn is based on hep-ph/0602028, with a minor fix
	double sigma = (exp_uncertainty <= 0.0) ? (1e-2*theory_uncertainty) : exp_uncertainty;
	double quad_err = std::hypot(sigma, theory_uncertainty);
	double tlim = (sigma/theory_uncertainty)*delta/quad_err;

	double exparg = -delta*delta/(2.0*quad_err*quad_err);
	double expterm = std::exp(exparg);

	double zterm2 = z_fn(delta/theory_uncertainty);
